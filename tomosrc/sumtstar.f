...
c   Subroutine that calculate l/(v*Q) for each segment of raypath     c
c   id = 1 for P; id = 2 for S                                        c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      SUBROUTINE sumtstar(id,maxpts,xarc,yarc,zarc,npts,ipt1,
     &                       grow,ifskip)
      parameter (nxmax = 45, nymax = 45, nzmax = 30)  ! max nodes

      integer maxpts, pt, ipt, ifskip, casep, k1D, k1Dv       ! max # internal points in raypath
      integer nnx, nny, nnz, nnxQ, nnyQ, nnzQ, nnzQ1D, nparm, lps
      real x(nxmax), z(nzmax), y(nymax)
      real xq(nxmax), zq(nzmax), yq(nymax), zq1D(nzmax)  ! node locations
      real*8 sn(2,nzmax,nymax,nxmax)
      real*8 attin(2,nzmax,nymax,nxmax), Qout(2,nzmax)  ! Syn 1/Q
      ...
      real*8 wc(8), wd(8), slc, wtc, snQ(8)     ! weighting coefficients
      real*8 Qx, Qy, Qz
      real*8 xlen, ylen, zlen, sslen, tstarin, tstarout
...
c      write(*,*) xq(1),xq(nnxQ),yq(1),yq(nnyQ),zq(1),zq(nnzQ)
c      write(*,*) "Earthquake:  ",xarc(0),yarc(0),zarc(0)
c      write(*,*) "First point: ",xarc(ipt),yarc(ipt),zarc(ipt)
      do i = 0, npts+2
        savexarc(i) = xarc(i)
        saveyarc(i) = yarc(i)
        savezarc(i) = zarc(i)
...
ccccc If the whole ray is within the Q model space, jump to buiding G
      if (ipt.eq.0) then
c        ipt = 1
c        write(*,*) "whole ray is within the Q model space"
        goto 235
      endif
...
c        write(*,*) "Earthquake:  ",xarc(0),yarc(0),zarc(0)
        write(*,*) "Station VAVP:  ",yarc(npts+2)
c        write(*,*) xarc(ipt),yarc(ipt),zarc(ipt)
c        write(*,*) xarc(ipt-1),yarc(ipt-1),zarc(ipt-1)
        ifskip = 1
        goto 237
...
c        endif
        ...
        k1Dv = 1
        do k = 1, nnz
          if (z(k) .lt. z0 .and. z(k+1) .ge. z0) k1Dv = k
        enddo
        if (z0 .eq. z(k1Dv+1) .and. sign .lt. 0.) k1Dv = k1Dv + 1
...
c        write(*,*) x0,y0,z0,sn(id,k0,j0,i0),k1D,Qout(1,25),Qout(2,k1D)
ccccc Adding grow elements corresponding to 1D Q model from zq(nnzQ) to z(nnz)
c          grow(nnxQ*nnyQ*nnzQ+k1D-nnzQ) = grow(nnxQ*nnyQ*nnzQ+k1D-nnzQ)+
c     &                    sslen*sn(id,k0,j0,i0)/Qout(id,k1D)
        if ((id .eq. 1) .or. (id .eq. 2)) then          ! Qp or Qs
          grow(nnxQ*nnyQ*nnzQ+k1D) = grow(nnxQ*nnyQ*nnzQ+k1D)+
     &                    sslen*sn(id,k0,j0,i0)/Qout(id,k1Dv)
        elseif (id .eq. 3) then    ! Qp/Qs
          grow(nnxQ*nnyQ*nnzQ+k1D) = grow(nnxQ*nnyQ*nnzQ+k1D)+
     &                sslen*sn(2,k0,j0,i0)/Qout(1,k1Dv)*Qpin1D(k1D)
        endif
        x0 = xss
        y0 = yss
        z0 = zss
...
c        if (z0 .eq. z(k0+1) .and. sign .lt. 0.) k0 = k0+1
     ...
     &            (abs(yq(ii)-y0) .lt. leny0)) then
        ...
        if (ylength .lt. sslength) then    ! subsegment ends at y(i)
          icase = 3
          sslength = ylength
        endif
        if (z0 .le. zq(1)) then
        ...
        if (zlength .lt. sslength) then    ! subsegment ends at z(j)
          icase = 4
        endif
c        write(*,*) icase, sslength, xlength, ylength, zlength
...
c     Weighting: c - to begin of segment, d - to end of segment
        iw = 0
        slc = 0.
        sld = 0.
        do jy = jq0, jq0+1
          do kz = kq0, kq0+1
            do ix = iq0, iq0+1
              ...
              if ((ix.eq.0).or.(jy.eq.0).or.(kz.eq.0)) then
                ...
                wc(iw) = sqrt(((xq(ix)-x0)*(1.-zq(kz)/re))**2 +
     &            ((yq(jy)-y0)*(1.-zq(kz)/re))**2 + (zq(kz)-z0)**2)
                wd(iw) =  sqrt(((xq(ix)-xss)*(1.-zq(kz)/re))**2 +
     &            ((yq(jy)-yss)*(1.-zq(kz)/re))**2 + (zq(kz)-zss)**2)
              endif
     ...
     &          d000+d001+d100+d101+d010+d011+d110+d111
        ...
        if (ipolar .eq. 1) then     ! x,y distances decr w/ depth
          xlen = xlen*(1.-(zss+z0)/(2.*re))
          ylen = ylen*(1.-(zss+z0)/(2.*re))    ! should there be a zlen here?
        endif
        sslen = sqrt((xlen**2) + (ylen**2) + (zlen**2))
        wtc = 0.5
c        write(*,*) "Length of the subsegment is ",sslen
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     &        sslen*sn000*attin(id,kq0,jq0,iq0)
          grow((jq0-1)*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0+1) =
     &        grow((jq0-1)*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0+1)+wtc*(d001+c001)
     &        *sslen*sn001*attin(id,kq0+1,jq0,iq0)
          grow(jq0*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0) =
     &        grow(jq0*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0) + wtc*(d010+c010)*
     &        sslen*sn010*attin(id,kq0,jq0,iq0+1)
          grow(jq0*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0+1) =
     &        grow(jq0*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0+1) + wtc*(d011+c011)*
     &        sslen*sn011*attin(id,kq0+1,jq0,iq0+1)
          grow((jq0-1)*nnzQ+nnyQ*nnzQ*iq0+kq0) =
     &        grow((jq0-1)*nnzQ+nnyQ*nnzQ*iq0+kq0) + wtc*(d100+c100)*
     &        sslen*sn100*attin(id,kq0,jq0+1,iq0)
          grow((jq0-1)*nnzQ+nnyQ*nnzQ*iq0+kq0+1) =
     &        grow((jq0-1)*nnzQ+nnyQ*nnzQ*iq0+kq0+1) + wtc*(d101+c101)*
     &        sslen*sn101*attin(id,kq0+1,jq0+1,iq0)
          grow(jq0*nnzQ+nnyQ*nnzQ*iq0+kq0) =
     &        grow(jq0*nnzQ+nnyQ*nnzQ*iq0+kq0) + wtc*(d110+c110)*
     &        sslen*sn110*attin(id,kq0,jq0+1,iq0+1)
          grow(jq0*nnzQ+nnyQ*nnzQ*iq0+kq0+1) =
     &        grow(jq0*nnzQ+nnyQ*nnzQ*iq0+kq0+1) + wtc*(d111+c111)*
     &        sslen*sn111*attin(id,kq0+1,jq0+1,iq0+1)
        else if (id .eq. 3) then    ! Qp/Qs
          grow((jq0-1)*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0) =
     &        grow((jq0-1)*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0)+wtc*(d000+c000)*
     &        sslen*sn000*attin(2,kq0,jq0,iq0)
     &        *Qpin(kq0,jq0,iq0)
          grow((jq0-1)*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0+1) =
     &        grow((jq0-1)*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0+1)+wtc*(d001+c001)
     &        *sslen*sn001*attin(2,kq0+1,jq0,iq0)
     &        *Qpin(kq0+1,jq0,iq0)
          grow(jq0*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0) =
     &        grow(jq0*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0) + wtc*(d010+c010)*
     &        sslen*sn010*attin(2,kq0,jq0,iq0+1)
     &        *Qpin(kq0,jq0,iq0+1)
          grow(jq0*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0+1) =
     &        grow(jq0*nnzQ+nnyQ*nnzQ*(iq0-1)+kq0+1) + wtc*(d011+c011)*
     &        sslen*sn011*attin(2,kq0+1,jq0,iq0+1)
     &        *Qpin(kq0+1,jq0,iq0+1)
          grow((jq0-1)*nnzQ+nnyQ*nnzQ*iq0+kq0) =
     &        grow((jq0-1)*nnzQ+nnyQ*nnzQ*iq0+kq0) + wtc*(d100+c100)*
     &        sslen*sn100*attin(2,kq0,jq0+1,iq0)
     &        *Qpin(kq0,jq0+1,iq0)
          grow((jq0-1)*nnzQ+nnyQ*nnzQ*iq0+kq0+1) =
     &        grow((jq0-1)*nnzQ+nnyQ*nnzQ*iq0+kq0+1) + wtc*(d101+c101)*
     &        sslen*sn101*attin(2,kq0+1,jq0+1,iq0)
     &        *Qpin(kq0+1,jq0+1,iq0)
          grow(jq0*nnzQ+nnyQ*nnzQ*iq0+kq0) =
     &        grow(jq0*nnzQ+nnyQ*nnzQ*iq0+kq0) + wtc*(d110+c110)*
     &        sslen*sn110*attin(2,kq0,jq0+1,iq0+1)
     &        *Qpin(kq0,jq0+1,iq0+1)
          grow(jq0*nnzQ+nnyQ*nnzQ*iq0+kq0+1) =
     &        grow(jq0*nnzQ+nnyQ*nnzQ*iq0+kq0+1) + wtc*(d111+c111)*
     &        sslen*sn111*attin(2,kq0+1,jq0+1,iq0+1)
     &        *Qpin(kq0+1,jq0+1,iq0+1)
        endif
        x0 = xss
        y0 = yss
        z0 = zss
        if (z0 .eq. zarc(ipt1)) then
          pt = pt+1
        elseif (icase .eq. 1) then      ! Move to next arc for case 1
          pt = pt + 1
          x1 = xarc(pt)
          y1 = yarc(pt)
          z1 = zarc(pt)
...
c        write(*,*) x0,y0,z0,pt,icase
      enddo       ! end of loop over each raypath node (pt .le. npts+1)
c      tstarin=0.
c      do i=1,nnxQ*nnyQ*nnzQ
c        tstarin=tstarin+grow(i)
c      enddo
c      write(*,*) "t* inside = ", tstarin

237   continue
c      write (*,*) 'ifskip = ',ifskip
      END
